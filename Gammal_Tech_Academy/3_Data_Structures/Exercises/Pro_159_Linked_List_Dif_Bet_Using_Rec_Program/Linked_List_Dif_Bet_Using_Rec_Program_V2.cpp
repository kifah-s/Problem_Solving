...
//* Linked List Difference Between Using Recursion Program (Version 2).
...
{
    cout << "\n\nYou welcome in Linked List Difference Between Using Recursion Program (Version 2) ..\n"
         << endl;
}
...
int differenceBetweenRecFun(Node *head)
{
    Node *temp = head;
    if (temp == NULL)
    {
    ...
    int evenSum = differenceBetweenRecFun(temp->next);
    return (temp->number % 2 == 0) ? evenSum + temp->number : evenSum - temp->number;
}
