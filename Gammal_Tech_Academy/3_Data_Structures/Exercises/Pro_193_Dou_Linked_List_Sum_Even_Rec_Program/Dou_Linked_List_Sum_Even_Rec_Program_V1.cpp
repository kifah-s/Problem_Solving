...
//* Double Linked List Sum Even Number Using Recursion Program (Version 1).
...
{
    cout << "\n\nYou welcome in Double Linked List Sum Even Number Using Recursion Program (Version 1) ..\n"
         << endl;
}
...
int sumEvenNumbersFun(Node *head)
{
    ...
    else
    {
        if (head->data % 2 == 0)
        {
            return head->data + sumEvenNumbersFun(head->next);
        }
        else
        {
            return sumEvenNumbersFun(head->next);
        }
    }
}
