...
//* Odd Elements Using Recursion Program (Version 1).
...
{
    cout << "\n\nYou welcome in Odd Elements Using Recursion Program (Version 1) ..\n"
         << endl;
}
...
int oddElementsRecFun(int size, int myArr[], int counterOddEle, int index = 0)
{
    if (index >= size)
    {
        return counterOddEle;
    }
    ...
    return oddElementsRecFun(size, myArr, counterOddEle, index + 1);
}
...
void resultFun()
{
    ...
    counterOddElements = oddElementsRecFun(sizeOfArray, myArray, counterOddElements);
