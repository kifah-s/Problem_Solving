

/* >>> Sum Using Function Program ( Version 4 ) <<< */

/* >>> A program that asks the user to enter two values, its function is that it collects the values that the user entered (using function). <<< */

#include <stdio.h>
#include <string.h>

void welcomeMassageFun();
int receiveValueFun(int *num1, int *num2);
int sumVariableFun(int *num1, int *num2);

int main()
{
    welcomeMassageFun();

    // Declare variables;
    int num1 = 0, num2 = 0;

    receiveValueFun(&num1, &num2);

    int sum = sumVariableFun(&num1, &num2);

    // Printing sum;
    printf("\nNumber 1 + Number 2 = %d\n\n", sum);

    return 0;
}

// Functions ..
// Welcome Massage Function;
void welcomeMassageFun()
{
    printf("\n\nYou welcome in Sum Using Function Program ( Version 4 ) ..\n\n");
}

// Receive value function;
int receiveValueFun(int *num1, int *num2)
{
    // Receive value from user;
    printf("Please enter two numbers : \n");
    printf("Number 1 : ");
    scanf("%d", &*num1);
    printf("Number 2 : ");
    scanf("%d", &*num2);
}

// sum Variable function;
int sumVariableFun(int *num1, int *num2)
{
    // Declare variables;
    int sumVariable = 0;

    // sum variables;
    sumVariable = *num1 + *num2;
}