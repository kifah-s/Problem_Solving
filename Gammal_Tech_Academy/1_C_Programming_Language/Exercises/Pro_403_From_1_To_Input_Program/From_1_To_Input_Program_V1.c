...
/* >>> A program that reads a number from a file and prints the numbers from 1 to the number in a second file. <<< */
...
int creatFile2ForReadFun(int numberInFile1, FILE *file2ForWrite)
{
    ...
    for (int i = 1; i <= numberInFile1; i++)
    {
        fprintf(file2ForWrite, "%d\n", i);
    }
