...
stDate CalculateVacationDaysReturnDate(stDate vacationStarts, short numberOfVacationDays)
{
    ...
    while (IsWeekEnd(vacationStarts))
    {
        vacationStarts = IncreaseDateByOneDay(vacationStarts);
    }

    for (short i = 1; i <= numberOfVacationDays + weekEndCounter; i++)
    {
        if (IsWeekEnd(vacationStarts))
        {
            weekEndCounter++;
...
int main()
{
    ...
    vacationEnds = CalculateVacationDaysReturnDate(vacationStarts, ReadNumberOfVacationDays());

    cout << "\nReturn Date: " << DayShortName(DayOfWeekOrder(vacationEnds)) << ", " << vacationEnds.day << "/" << vacationEnds.month << "/" << vacationStarts.year << endl;
