...
enDistinct IsDistinctNumber(int num, int arr[10], int arrLen)
{
    for (int i = 0; i < arrLen; i++)
    {
        if (num == arr[i])
        {
            return enDistinct::notDistinct;
        }
    }

    return enDistinct::distinct;
}
...
void CopyArray(int arr1[100], int arr2[100], int arr1Length, int &arr2Length)
{
    ...
    {
        if (IsDistinctNumber(arr1[i], arr2, arr2Length) == enDistinct::distinct)
        {
