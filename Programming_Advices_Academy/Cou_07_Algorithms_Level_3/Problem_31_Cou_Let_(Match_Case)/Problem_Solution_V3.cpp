...
int CountCharacter(string myString, char myChar, enUpperOrSmall UpperOrSmall)
{
    int counter = 0;

    for (int i = 0; i < myString.length(); i++)
    {
        if (myString[i] == myChar && UpperOrSmall == enUpperOrSmall::small)
        {
            counter++;
        }
...
int main()
{
    ...
    myString = ReadString();
    myChar = ReadChar();
    countCharacter = CountCharacter(myString, myChar, enUpperOrSmall::small);
