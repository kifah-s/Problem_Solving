...
void AddEachRowSumToAdditionArray(int arr[3][3], short rows, short cols, int addArr[3])
{
    for (short i = 0; i < rows; i++)
    {
        addArr[i] = RowSum(arr, i, cols);
    }
}
...
void PrintAdditionArray(int addArr[3], short rows)
{
    ...
    for (short i = 0; i < rows; i++)
    {
        cout << " Row " << i + 1 << " Sum = " << addArr[i] << endl;
    }
}
...
int main()
{
    ...
    AddEachRowSumToAdditionArray(array, 3, 3, additionArray);
